...
public class MultiAgentSequentialExample
{
    public static async Task Main(string[] args)
    {
        ...
        var optimizacionDistribucionAgent = new ChatCompletionAgent
        {
            ...
            Instructions =
                """
                Si se recomienda una reposición, tu respuesta debe ser exactamente:
        ...
        // (opcional) intentar parsear a int para mostrar algo más bonito
        int.TryParse(stockTexto.Trim(), out int stock);

        // PASO 2: DECISIÓN DE REPOSICIÓN
        Console.WriteLine("[2] Predicción de reposición");
        string promptReposicion =
            $"El stock actual del producto es {stock}. Indica si se necesita reposición según tus instrucciones.";
        string decisionReposicion = await EjecutarAgenteUnaVez(prediccionReposicionAgent, promptReposicion);

        Console.WriteLine($"Respuesta AgenteReposicion: {decisionReposicion}\n");

        // PASO 3: ESTRATEGIA DE DISTRIBUCIÓN
        Console.WriteLine("[3] Optimización de distribución");
        string promptDistribucion =
            $"Basándote en esta recomendación: \"{decisionReposicion}\", indica la estrategia de distribución.";
        string estrategiaDistribucion = await EjecutarAgenteUnaVez(optimizacionDistribucionAgent, promptDistribucion);

        Console.WriteLine($"Respuesta AgenteDistribucion: {estrategiaDistribucion}\n");

        // --- RESULTADO FINAL ---
        Console.WriteLine("=== RESUMEN FINAL DE LA ORQUESTACIÓN SECUENCIAL ===");
